    ...
    BTCBlock StratumPool::getNewBlock()
    {
        ...
        if(msg.getType() == stratum::MsgType::NOTIFY)
        {
            ...
            b.extranonce2_size = this->stratum_pool.getExtranonce2Size();
                    ...
                    char *ver = ver_;
                    char *nprevhash = prevhash_;
                    char *ntime = ntime_;
                    char *nbits = nbits_;
                    char *nonce = nonce_;

               /*
                    ver = (char*)b.version.c_str();
                    nprevhash = (char*)b.prevhash.c_str();
                    ntime = (char*)b.ntime.c_str();
                    nbits = (char*)b.nonce.c_str();
              
  */
                    strncpy(ver , b.version.c_str(),b.version.length());
                    strncpy(nprevhash , b.prevhash.c_str(),b.prevhash.length());
                    strncpy(ntime , b.ntime.c_str(),b.ntime.length());
                    strncpy( nbits , b.nbits.c_str(),b.nbits.length());
                   // strncpy( nonce , b.nonce.c_str(),b.nonce.length());
                    
                  
                                   // merkle_branch 
                    std::vector<char*>  vc;
                    std::transform(b.merkle_branch.begin(), b.merkle_branch.end(), std::back_inserter(vc), convert);   
                    tx = vc.size();
                        ...
                        for(int i=0;i<tx;++i)
                              {
                                  ...
                                  strncpy(merkle_branch[i],merkle_branch[i], 65);
                                  merkle_branch[i][64] = '\0';
                              }

                    for ( size_t i = 0 ; i < vc.size() ; i++ )
                          strcpy(merkle_branch[i], vc[i] );

                          
                b.hash(ver, nprevhash, ntime, nbits, nonce, tx , merkle_branch);
             }
    ...
    {/*
       Serial.println( b.job_id.c_str());
        Serial.println( b.extranonce2.c_str());
        Serial.println( b.nonce.c_str());
        Serial.println( b.ntime.c_str());
      */
